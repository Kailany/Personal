        ...
        if (cancel) {
            ...
            val stringRequest = StringRequest(Request.Method.GET, url,
                    Response.Listener<String> { response ->
                        ...
                        val users = result.getJSONArray("usuarios")
                        for (i in 0..(users.length() - 1)) {
                            val user = users.getJSONObject(i)

                            if (emailStr.equals(user.getString("usu_email")) &&
                                    passwordStr.md5().equals(user.getString("usu_senha"))) {
                                encontrou = true
                        ...
                        if (!encontrou) {
                            Toast.makeText(this, "Dados não conferem!", Toast.LENGTH_LONG).show()
                        }
                    },
                    Response.ErrorListener {
                        showProgress(false)
                        Toast.makeText(this, "that didnt work", Toast.LENGTH_SHORT).show()
                    })
